...
The pipeline is **industry- and country-agnostic**. All client/industry/geo specifics live in
`data/campaigns/{campaign_id}/` — never in code defaults or shared scripts.
...
- **Deterministic funnel:** `run_seed_source.py` → `match_campaign_icp.py` →
 ...
 Post-run: `open_campaign_cost_dashboard.py`.
- Campaign scaffolding: `icp.json` (+ `website_analysis_mode`) + `seed_sources.json` + `stages/` + `run_payload.json`.
...
- LLM for website stage only: **Gemini 3.7 Flash → gpt-5-mini** (`WEBSITE_PROVIDERS`). No LLM company prefilter or LLM qualify.
...
- Firecrawl: map + crawl (Apify fallback) + careers probe; max ~5 pages per company in normal operation.
...
- Cost: paid stage CLIs auto-append `cost_runs.json`; prefer `apify_truth_usd` over CostTracker for Apify billing.
